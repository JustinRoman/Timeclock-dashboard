...
<head>
    ...
    <title>Welcome Admin | Inactive employees</title>
        ...
        <div class="container-fluid">
            <?php
                if (isset($_GET['action']) && $_GET['action']=='delete') {
                    ...
                    $temp = $conn->prepare("DELETE FROM users WHERE id = ?");
                    $temp->execute([$id]);
                } else if (isset($_GET['action']) && $_GET['action']=='restore') {
                    $id = $_GET['id'];

                    $restore_employee = $conn->prepare("UPDATE users SET user_type = '1' WHERE id = ?");
                    $restore_employee->execute([$id]);
                }
             ?>
        <!-- Striped Rows -->
                        ...
                        <div class="body table-responsive">
                            <table class="table table-striped dataTable js-exportable">
                                <thead>
                                    ...
                                    <?php
                                        $administrator_list = $conn->prepare("SELECT id, name, email, dept, office FROM users WHERE user_type = '2'");
                                                ...
                                                $email = $row['email'];
                                            {
                                                    ...
                                                    echo '
                                                        <tr>
                                                            ...
                                                            <td>'.$office.'</td>
                                                            <td>
                                                                <a data-placement="top" title="View timesheet" href="#" type="button" class="btn btn-default waves-effect">
                                                                    <i class="material-icons">perm_contact_calendar</i>
                                                                </a>
                                                                <a onclick="return confirm(\'Are you sure you want to restore this employee?\')" data-placement="top" title="Restore employee" href="inactive_employees.php?action=restore&id='.$id.'" type="button" class="btn btn-default waves-effect">
                                                                    <i class="material-icons">settings_backup_restore</i>
                                                                </a>
                                                                <a onclick="return confirm(\'Are you sure you want to permanently remove this employee?\')" data-placement="top" title="Remove employee" type="button" href="inactive_employees.php?action=delete&id='.$id.'" class="btn btn-danger waves-effect" data-type="with-title">
